...
const DeleteInsurant = (insurant, factory) => {
   ...
   */
  const isActiveCompany = ({ id: deleteId }, { id: activeId }) =>
    deleteId === activeId;
  /**
  ...
  if (isActiveCompany(insurant, factory.companyObj)) {
    if (factory.polisObj.insurants.length > 1) {
      const newActive = chooseActiveCompany(
        insurant,
        factory.polisObj.insurants
      );
  ...
  factory.polisObj.insurants = factory.polisObj.insurants.filter(
    el => el !== insurant
  );
};
...
function getWords(num = 0, type = "number", firstRow) {
  ...
  function num_letters(num, d) {
    ...
    for (let i = num.length - 1; i >= 0; i--) {
      ...
      if (num[i] != "000") {
        str =
          (((d && i == num.length - 1) || i == num.length - 2) &&
          (num[i][2] == "1" || num[i][2] == "2")
            ? t(num[i], 1)
            : t(num[i])) +
          declOfNum(
            num[i],
            arr1[0][num.length - 1 - i],
            i == num.length - 2 ? arr1[1] : arr1[2]
          ) +
          str;
      }
    }
    function t(num, d) {
      ...
      return (
        ...
        (num[1] == 1
          ? arr2[1][num[2]]
          : arr2[2][num[1]] + (d ? arr2[4][num[2]] : arr2[0][num[2]]))
      );
    }
    return str;
  }
  function declOfNum(n, t, o) {
    ...
    return t == ""
      ? ""
      : " " + t + (n[n.length - 2] == "1" ? o[2] : o[num[n[n.length - 1]]]);
  }
  function razUp(arr) {
  ...
  function sum_letters(num) {
    num = Number(num)
      .toFixed(2)
      .split("."); // округлить до сотых и сделать массив двух чисел: до точки и после неё
    switch (type) {
      case "number":
        return razUp(num_letters(num[0]));
      case "price":
        return razUp(
          num_letters(num[0]) + declOfNum(num[0], "рубл", ["ь", "я", "ей"])
        );
    }
  }
}
function GetAllPackages(obj) {
  const onlyRisks = obj.filter(x => x.model === "risk");
  const packs = [];
  onlyRisks.forEach(risk =>
    risk.values.forEach(val => {
      if (val.action === "package") packs.push(val);
    })
  );
  return packs;
}
