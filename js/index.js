
// require ('./lib/angular-route.js');
// require ('./lib/angular-cookies.min.js');

import angular from './lib/angular.min.js';
require ('./app.js');
require ('./proto.js');
require ('./calculation.js');
require ('./controllers/calculationCtrl.js');
require ('./controllers/loginCtrl.js');
require ('./controllers/dashboard.js');
require ('./controllers/matrixCtrl.js');
require ('./controllers/search.js');
require ('./controllers/HIP.js');
require ('./controllers/companyCtrl.js');
require ('./controllers/polisCtrl.js');
require ('./controllers/polisEditorCtrl.js');
require ('./controllers/financeCtrl.js');
require ('./controllers/profileCtrl.js');
require ('./polismaker.js');

import '../css/style.css'
