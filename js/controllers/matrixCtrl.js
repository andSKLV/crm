...
app.controller("matrixCtrl", function(
  $rootScope,
  $http,
  myFactory,
  $timeout,
  ...
  this.deleteCalculation = function(row) {
    ...
    data.id = id;
    return $http.post("php/save.php", data).then(
      function success(response) {
        if (response.data === "Успешно удалено")
          console.log("calculation successfully deleted");
  ...
  this.deleteLink = calc => {
    ...
    return $http.post("php/save.php", query).then(
      resp => {
   ...
   * @param {number} id
   */
  this.loadCalculation = function(id) {
    //нажимаем на строку расчета в результате поиска
    myFactory.calcObj = {};
    clearSearch();
    $timeout(async function() {
      ...
      data.id = id;
      let scope = this;
      $http.post("php/search.php", data).then(
          ...
          if (response.data.user_name == 123) {
            mass.forEach(function(park) {
              ...
              park.processes.forEach(function(process) {
                ...
                array.push(proc);
                if (
                  process.multi !== undefined &&
                  multi.indexOf(process.multi) == -1
                )
                  multi.push(process.multi);
              });
              ...
              for (let key in park) {
                //копируем все свойства парка в созданный парк
            ...
            if (
              multies.length > 0 &&
              myFactory.multi.multies.length === multies.length
            ) {
            ...
            if (
              response.data.HIPname !== "" &&
              response.data !== "Перевозчики"
            ) {
            ...
            if (
              response.data.a_limit != 0 &&
              response.data.a_limit != myFactory.a_limit.max_limit
            ) {
              //агрегатный лимит
            ...
            if (
              ...
              response.data.payment != myFactory.payment.val
            ) {
              //этапы платежей
            ...
            if (
              response.data.fact_premia != ";Р" &&
              response.data.fact_premia != ";1"
            ) {
              ...
              Math.round(myFactory.totalPrice) != response.data.total_price
            ) {
            ...
            for (let key in processes) {
              ...
              if (process.amount[1] == "Тягачей")
                process.amount = process.amount[0] * 24;
            ...
            mass.forEach(function(multi) {
              if (multi.trs != "") {
                ...
                for (let i = 0; i < multi.trs.length; i++) {
                  if (multi.trs[i] == "")
                    multi.trs.splice(multi.trs.indexOf(multi.trs[i]), 1);
                }
                ...
                if (multi.package_name) {
                  ...
                  multi.trs.forEach(function(tr) {
                    ...
                    if (
                      multi.multi &&
                      multiMass[multi.multi].processes.indexOf(tr) == -1
                    ) {
                  ...
                  if (multi.multi)
                    multiMass[multi.multi].package = multi.package_name;
                } else {
                  multi.trs.forEach(function(tr) {
            ...
            for (let i = 0; i < parks.length; i++) {
              ...
              mass.forEach(function(process) {
                ...
                if (process.packageNum !== undefined)
                  proc.packageNum = process.packageNum;
                if (process.multi !== undefined) proc.multi = process.multi;
                if (process.package !== undefined)
                  proc.package = process.package;
                if (process.koef !== undefined)
                  proc.practicalPriceKoef = process.koef;
                array.push(proc);
              });
            ...
            if (practicalPrice[0] != "undefined") {
              ...
              response.data.total_price != 0 &&
              myFactory.totalPrice != response.data.total_price
            ) {
              myFactory.practicalPrice.val = response.data.total_price * 1;
              myFactory.practicalPrice.koef =
                myFactory.practicalPrice.val / myFactory.totalPrice;
  ...
  this.loadCompany = function(id, noRelocation) {
    ...
    data.id = id;
    return $http.post("php/search.php", data).then(
      async resp => {
        const loadAddresses = () => {
          const check = str => {
          ...
          return $http.post("php/load.php", query).then(
            resp => {
              ...
              if (data[0].id !== "1") {
                ...
                myFactory.newClientCard["Доп. информация"][
                  "Юридический адрес"
                ] = legal;
              }
              if (data[1].id !== "1") {
    ...
    return $http.post("php/save.php", save).then(
      async function success(response) {
        if (isNaN(Number(response.data))) {
          alert(
            "Ошибка при пересохранении расчета. Пожалуйста, по возможности не закрывайте окно и обратитесь к разработчику"
          );
