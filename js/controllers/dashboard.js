...
app.controller("dashboardCtrl", function(
  ...
  this.clickedOnTopOfDashboard = (value, param) => {
    ...
    if (isNew) {
      //если нажата кнопка "новый", проверяем, чтобы не было уже созданного объекта, иначе он сотрется
      switch (value.name) {
        case "Расчет":
    ...
    if (
      param.name === "Найти" &&
      value.name === "Расчет" &&
      this.myFactory.parks.length > 0
    )
      return false; // убираем возможность поиска при существующем расчете
  ...
  $http.post("./src/dashboard.json").then(
    function success(response) {
      //устанавливаем каретку управления и заполняем ее из файла dashboard.json
      scope.currObj = response.data;
      scope.myFactory.keyCodes.qwerty.length = scope.currObj.filter(function(
        obj
      ) {
  ...
  this.relocatePage = function(value) {
    ...
    if (value.urlTo === "profile" && !this.myFactory.companyObj.id)
      value.urlTo = "company";
  ...
  this.currentProcess = {};
  this.selectParam = function(index) {
    // нажатии на nav
  ...
  this.toggleMenu = function(toBeClosed) {
    ...
    if (toBeClosed === undefined)
      menu.classList.toggle("nav-back_container--hidden");
    if (toBeClosed === false)
      menu.classList.aremovedd("nav-back_container--hidden");
