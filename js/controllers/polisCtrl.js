...
app.controller("polisCtrl", function(
  myFactory,
  $http,
  $location,
  $scope,
  ...
  $scope.init = async () => {
    ...
    const selectNames = () => {
      ...
      if (
        myFactory.calculationName !== "" &&
        myFactory.calculationName !== undefined
      )
        this.calculationName = myFactory.calculationName;
    ...
    const loadDashboardObj = () => {
      ...
      return $http.post("./src/polis.json").then(
        resp => {
          if (!Array.isArray(resp.data) || resp.data.length < 1) {
            alert(
              "Возникла ошибка при загрузке данных. Пожалуйста, по возможности не закрывайте окно и обратитесь к разработчику"
            );
            console.error(resp);
          } else {
    ...
    const openTab = () => {
      ...
      if (myFactory.cameFrom.name === "Редактор полиса") {
        ...
        tabIndex = $scope.currObj.findIndex(
          val => val.name === "Оговорки и условия"
        );
        ...
        myFactory.cameFrom.name === "Редактор карты клиента" ||
        myFactory.cameFrom.name === "Карту клиента"
      ) {
    ...
    if (
      !myFactory.polisObj ||
      !myFactory.polisObj.isRequested ||
      !$scope.currObj ||
      $scope.currObj.length === 0
    ) {
      if (!myFactory.polisObj.conditions && myFactory.parks.length > 0) {
        const type = myFactory.calcObj.factory
          ? myFactory.calcObj.factory.HIPname
          : "Перевозчики";
    ...
    if (
      ...
      await myFactory.polisObj.loadConditions(
        myFactory.calcObj.factory.HIPname,
        baseRiskNeeded
      );
      myFactory.polisObj.type = myFactory.calcObj.factory.HIPname;
    ...
    if (
      !$scope.myFactory.polisObj.dates.start &&
      !$scope.myFactory.polisObj.dates.end
    )
      setInitialDates();

    myFactory.polisObj.updateConditionsCheck();
    // нужно ли обновить оговорки, машины, финансы после перехода
    const needRefreshCalc =
      myFactory.parks.length > 0 &&
      (myFactory.cameFrom.path === "/calculation" ||
        !myFactory.parks[0].processes[0].cars);
    if (needRefreshCalc) $scope.updateState();

    const needToClearState =
      myFactory.parks.length === 0 &&
      (myFactory.payment.array && myFactory.payment.array.length > 0);
    if (needToClearState) $scope.clearState();
    openTab();

    //добавляем открытую компанию в сострахователи
    if (
      myFactory.companyObj.id &&
      !myFactory.polisObj.insurants.some(
        ins => ins.id === myFactory.companyObj.id
      )
    ) {
      if (myFactory.polisObj.insurants.length === 4)
        $scope.deleteInsurant(myFactory.polisObj.insurants[0]);
  ...
  $scope.changeCar = (process, car, group) => {
    ...
    if (process.cars.includes(nextCar)) {
      ...
      [process.cars[oldCarIndex], process.cars[nextCarIndex]] = [
        process.cars[nextCarIndex],
        process.cars[oldCarIndex]
      ];
    } else {
  ...
  $scope.calcFinances = () => {
    ...
    if (
      myFactory.parks.length === 0 ||
      !(myFactory.polisObj.dates.start || myFactory.polisObj.dates.end)
    )
      return false;
    const price =
      myFactory.practicalPrice.val === "" || myFactory.practicalPrice.val == 0
        ? myFactory.totalPrice
        : myFactory.practicalPrice.val;
    if (!myFactory.payment.array)
      myFactory.payment.makeArray(price, myFactory.polisObj.dates);
    else {
      let needToCreate = false;
      const payTotal =
        typeof myFactory.payment.totalPrice === "string"
          ? myFactory.payment.totalPrice
          : addSpaces(Math.round(myFactory.payment.totalPrice));
  ...
  $scope.newDashboard = {
    ...
    setCurrentPage(index) {
      ...
      if (
        index === 4 &&
        $scope.myFactory.payment.array &&
        $scope.myFactory.payment.array.length > 0
      )
        $scope.myFactory.polisObj.financeSeen = true;
    ...
    alreadySelected(index) {
      switch (index) {
        ...
        case 3:
          return (
            myFactory.polisObj.dates.start &&
            myFactory.polisObj.dates.end &&
            myFactory.polisObj.datesSeen
          );
          break;
        case 4:
          return (
            myFactory.polisObj.financeSeen &&
            myFactory.payment.array &&
            myFactory.payment.array.length > 0
          );
          break;
        case 5:
  ...
  this.makePDF = () => {
    ...
    /**
     * Функция загрузки рисков. Еще раз, зачем?
     */
    const getRisks = () => {
        ...
        data.forEach(({ model, values }) => {
          if (model == "risk") {
            values.forEach(value => {
              if (value.action === undefined && value.type === "risk")
                risks.push(value);
            });
          }
        });
  ...
  $scope.deleteAdditionFromDB = id => {
    ...
    query.id = id;
    $http.post("./php/save.php", query).then(
      resp => {
        if (resp.data !== "success") {
          alert(
            "Ошибка удаления. Пожалуйста, по возможности не закрывайте окно и обратитесь к разработчику"
          );
          console.error(resp.data);
        } else
          $rootScope.search_result = $rootScope.search_result.filter(
            val => val.id !== id
          );
      },
      err => {
  ...
  $scope.endChange = control => {
    ...
    const BTN_SWITCH_TODAY = $scope.currObj[
      $scope.newDashboard.currentPage
    ].values.filter(
      val => val.type === "btn-switch" && val.control === "today"
    )[0];
    const BTN_SWITCH_TIME = $scope.currObj[
      $scope.newDashboard.currentPage
    ].values.filter(
      val => val.type === "btn-switch" && val.control === "time"
    )[0];
    switch (control) {
      case "start":
  ...
  $scope.loadCompanyProfile = async function(id) {
    ...
    if (
      ...
      !myFactory.companyObj ||
      !myFactory.companyObj.id
    )
      $scope.changeLocation("Компания");
    else {
      myFactory.companyObj.id = id;
  ...
  $scope.updateState = async id => {
    ...
    const calcIsLoaded = async () => {
      function check() {
        return id
          ? myFactory.calcObj.id === id
          : myFactory.parks.length > 0 && myFactory.calcObj.isInited;
      }
  ...
  $scope.onClickConfigCheckbox = name => {
    myFactory.polisObj.docsIncluded[name] = !myFactory.polisObj.docsIncluded[
      name
    ];
  };
