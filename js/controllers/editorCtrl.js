app.controller('editorCtrl', function ($scope, $rootScope, $http, $q, $location, myFactory) {
        ...
        allRisks.forEach(host => {
            host.values.forEach(val => {
                if (val.type === 'risk' && !val.action) {
                    (val.baseRisk) ? $scope.editor.risksReservedNames.push(val.name) : $scope.addRiskToPool(val.name);
                }
            })
        })
    };
    ...
    $scope.removeRiskFromPool = name => {
        $scope.editor.risksReservedNames = $scope.editor.risksReservedNames.filter(r => r !== name);
    ...
    $scope.editRiskInPool = (from, to) => {
        $scope.editor.risksReservedNames = $scope.editor.risksReservedNames.map((rName) => (rName === from) ? to : rName);
