...
async function loadDB() {
  try {
    ...
    alert(
      "Проблемы с загрузкой базы данных. Работа в тестовом режиме. Обратитесь к разработчику"
    );
    console.error(e);
...
function initDB(resp) {
  ...
  for (let i = 0; i < ar.length; i++) {
    if (ar[i]["number"] * 1 == 1)
      Points.amount.push([ar[i]["x"] * 1, ar[i]["y"] * 1]);
    else if (ar[i]["number"] * 1 == 2)
      Points.cost.push([ar[i]["x"] * 1, ar[i]["y"] * 1]);
    else if (ar[i]["number"] * 1 == 3)
      Points.risk.push([ar[i]["x"] * 1, ar[i]["y"] * 1]);
    else if (ar[i]["number"] * 1 == 4)
      Points.payment.push([ar[i]["x"] * 1, ar[i]["y"] * 1]);
...
function SplineKoeff(index, mass) {
  ...
  if (num < 3) {
    ...
    if (num == 3) {
      ...
      B[n] =
        (mass[n][1] - mass[NM1][1]) / D[NM1] + D[NM1] * (C[NM1] + 2 * C[n]);
      for (let i = 0; i < NM1 + 1; i++) {
        B[i] =
          (mass[i + 1][1] - mass[i][1]) / D[i] - D[i] * (C[i + 1] + 2 * C[i]);
        D[i] = (C[i + 1] - C[i]) / D[i];
        C[i] = 3 * C[i];
      }
      ...
      C[0] =
        C[2] / (mass[3][0] - mass[1][0]) - C[1] / (mass[2][0] - mass[0][0]);

      C[n] =
        C[n - 1] / (mass[n][0] - mass[n - 2][0]) -
        C[n - 2] / (mass[n - 1][0] - mass[n - 3][0]);
...
function Spline(U, mass, index) {
  ...
  if (U > mass[mass.length - 1][0]) {
    ...
    alert(
      "Стоимость превышает максимальное значение графика. Значение коэффициента взято с последней точки"
    );
  ...
  if (mass[i][0] > U) {
    ...
    let Spline1 =
      mass[i][1] +
      dx * (MB[index][i] + dx * (MC[index][i] + dx * MD[index][i]));
    return Spline1;
  }
