...
app.directive("editorNav", function() {
  return {
    restrict: "A",
    templateUrl: "./templates/paths/editor/navigation.html"
  };
});
...
app.directive("paramInfo", function() {
  return {
    restrict: "A",
    templateUrl: "./templates/paths/editor/paramInfo.html"
  };
});
        ...
        try {
          ...
          reader.onload = function(e) {
            ...
            try {
              ...
              $scope.changeE.target.parentNode.classList.toggle(
                "select--hidden"
              ); //закрываем попап с выбором файла
              ...
              alert(
                "Выбран несоответствующий файл. Пожалуйста, выберите файл формата xls или xlsx"
              );
              console.error(e);
            }
          };
  ...
  function prepareList(list) {
    ...
    for (let i = startRow; i <= endRow; i++) {
      const car = {
        ...
        insurant:
          list[`F${i}`] && list[`F${i}`].w.match(/\d/) ? list[`F${i}`].w : ""
      };
      cars.push(car);
    }
    return cars;
    ...
    link: function($scope, $element, $attrs, ctrl) {
      ...
      $element.bind("blur", $event => {
        if ($attrs["currencyInput"] == "a_limit") {
          ...
          if (![1, 2, 4, 6, 12].includes(Number($element.val())))
            $element.val(1);
    ...
    payment: {
      ...
      makeArray(price, { start = "", end = "", time = "", startDate = "" }) {
        ...
        const calcPrices = (price, payments) => {
          ...
          res.last = addSpaces(
            roundToFixed(price - res.month * payments, 2) + res.month
          );
          return res;
        };
