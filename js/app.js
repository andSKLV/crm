      ...
      resolve: {
        'check': function ($location, $cookies, $rootScope, $http) {
          ...
          if ($rootScope.loggedIn) {
            ...
            $http.post('php/authorization.php', data)
        ...
        if ($attrs['currencyInput'] == 'a_limit') {
          ...
          if (key == 13) {
            ...
            if (myFactory.process.constructor.name == 'Park') {
              ...
              if ($attrs['param'] == 'amount' && myFactory.amountType == 'Тягачей') {
                myFactory.process.changeProperty($attrs['param'], val * 24);
              } else {
                myFactory.process.changeProperty($attrs['param'], val);
              }
            }
            else {
              if ($attrs['param'] == 'amount' && myFactory.amountType == 'Тягачей') {
                ...
                myFactory.process[$attrs['param']] = val;
              }
            }
            if ($scope.calculation.karetka.mode == 'making new process') {
              ...
              for (let key in myFactory.process) {
                if (myFactory.process[key] === '') {
                  ...
                  target.blur();
                  document.querySelector('.dashboard_container')
                    .focus();
...
app.factory('myFactory', function () {
  return {
      ...
      if (this.multi.arrays.wrapping.length === 1 && this.multi.arrays.risk.length === 1) {
        ...
        if (packages.length > 0) {
          ...
          if (bindMulti) {
            ...
            this.multi.multies.push(new Multi(obj.array, obj.packName, obj.template));
          }
          return obj.array;
        }
        else {
      ...
      if (mode == 'changing') {
        ...
        function enrollRisks({ packages, notPackages }) {
          ...
          packages.forEach(pack => {
            ...
            const thisPack = this.packages.find(p => p.name === pack)
              .values
              .map(val => {
    ...
    deleteProcess(process) {
      if (process.multi) {
        ...
        if (process.multi.processes.length < 2) {
          let newMulti;
          if (process.multi.prevMulti) {
            newMulti = process.multi.prevMulti;
      ...
      // если процесс единственный в парке, удаляем парк
        ...
        this.parks.splice(this.parks.indexOf(process.park), 1);
      }
    },
    /**
    ...
    async loadLinks(type, id) {
      ...
      fd.append('type', type);
      fd.append('id', id);
      const req = new Request('php/get_link.php', {
        method: 'POST',
        body: fd
      });
      return fetch(req)
